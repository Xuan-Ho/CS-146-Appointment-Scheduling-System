...
public class AppointmentCalendar{
  ...
  public static void display(){
      ...
      //Prepare frame
      frmMain = new JFrame ("Appointment Calendar"); //Create frame
      ...
      //Set border
      pnlCalendar.setBorder(BorderFactory.createTitledBorder("Double-Click on a Date"));
      	  ...
      	  public void mouseClicked(MouseEvent e) {
      		  Integer selDateInt = 0;
      		    if (e.getClickCount() == 2) {
      		      ...
      		      int month = Integer.parseInt(getMonth(lblMonth.getText()));
      		      int day = Integer.parseInt(target.getValueAt(target.getSelectedRow(), target.getSelectedColumn()).toString());
	    			  
      		      mDate.set(year,month,day);
      		      
      		      //System.out.println(hash(mDate));
      		      AppointmentDay.display();
  ...
  // Hash function takes a date and returns a unique index
  public static long hash( Calendar key ){
  	int tableSize = 730; // table size required to store 2 years worth of dates
  	
		  
		Calendar aYearAgo = Calendar.getInstance();
		aYearAgo.add(Calendar.DAY_OF_MONTH, -365);
		
	    Date startDate = aYearAgo.getTime();
	    Date endDate = key.getTime();
	    long startTime = startDate.getTime();
	    long endTime = endDate.getTime();
	    long diffTime = endTime - startTime;
	    // diffDays stores the number of days between a year from today's date and the given date
	    long diffDays = diffTime / (1000 * 60 * 60 * 24); 
	    // if date is less than a year ago, return index 0
  	if (diffDays < 0){
  		return 0;
  	}
  	// if date is greater than a year ago, return last index
  	if (diffDays > tableSize){
  		return tableSize-1;
  	}    	
  	return diffDays;
  }
  // takes the name of a month and returns the month number
